...
export const SocialMedia: React.FC<SocialMediaProps> = ({ icon, text, onClick }) => {
  return (
    <div className={`flex gap-1 sm:gap-4 self-stretch my-auto max-auto max-w-[18%] hover:text-violet-400 active:text-violet-400 rounded-lg hover:bg-slate-900 active:bg-slate-800 cursor-pointer ${koho.className} text-[0.5rem] sm:text-xl`}
    onClick={onClick}>
      <img
        loading="lazy"
        src={icon}
